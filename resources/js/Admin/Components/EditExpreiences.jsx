...
export default function EditExpreiences(props) {
    const initialDtate = "";
    ...
    const getExpreiences = () =>{
        Axios.get('experiences/'+ props.editid).then(response => {
            if(response.status == 200){
                console.log(response)
                setTitle(response.data.title)
                setaboutex(response.data.description)
    ...
    const updateExpreiences = (data) =>{
        Axios.put('experiences/'+props.editid, data).then(response => {
            if(response.status == 200){
                console.log(response)
    ...
    const registerExpreiences = e =>{
        ...
        const data = {title, aboutex, start_year, end_year}
        updateExpreiences(data)
    }

    return (
        <>
           <h5 className="text-center text-2xl text-green font-bold p-3">Edit Expreience Item</h5>
